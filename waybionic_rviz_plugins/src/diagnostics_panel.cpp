...
QHeaderView::section {
  ...
  void DiagnosticsPanel::onInitialize()
  {
    if (auto ros_node_abstraction = getDisplayContext()->getRosNodeAbstraction().lock())
    {
      ...
      motion_command_publisher_ = rviz_node_->create_publisher<std_msgs::msg::String>(
  ...
  void DiagnosticsPanel::buildMovementTestUi(QVBoxLayout *root_layout)
  {
    ...
    connect(movement_test_button_, &QPushButton::clicked, this, [this]()
            {
    if (!motion_command_publisher_) {
      movement_test_status_->setText("ROS connection is not ready.");
      return;
    }

    std_msgs::msg::String command;
    command.data = "RUN";
    motion_command_publisher_->publish(command);
    movement_test_status_->setText("Movement test command sent."); });
  ...
  void DiagnosticsPanel::updateAlerts(const std::vector<DiagnosticMessage> &messages)
  {
    ...
    for (const auto &message : messages)
    {
      ...
      label->setStyleSheet(QString(
                               ...
                               "font-weight: 800;"
                               "padding: 8px;")
                               .arg(statusColor(message.status)));
